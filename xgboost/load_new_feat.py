...
def load_feat():
    ...
    #                 'followers_num', 'following_num', 'content_length']

    all_feat_list = ['content_length', 'hashtag', 'late_night_times', 'figure_at', 'reputation', 'followee_num', 'followers_num', 'figure_at_every', 'following_num', 'figure_at_sigle', 'active_day_ratio', 'url', 'time_interval_mean', 'test_similarity', 'figure_RRT', 'follow_ratio', 'day_interval_variance', 'text_similarity', 'time_interval_var', 'average_comm', 'post_num', 'mention']
    
    
    raw_file = pd.read_json(open(spammer_order), orient='records')

    idx_title = ['id'] + all_feat_list + ['is_spammer']
    raw_file.rename(columns={i: idx_title[i] for i in range(len(idx_title))}, inplace=True)

    aval_feat_list = ['hashtag', 'reputation', 'post_num', 'mention', 'test_similarity', 'url', 'time_interval_mean',
                     'time_interval_var',  'followers_num', 'following_num', 'content_length']
